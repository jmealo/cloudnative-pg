...
#   GINKGO_NODES               — ginkgo workers (default: auto = min(cluster nodes, 5))
#   GINKGO_MAX_NODES           — cap for auto worker selection (default: 5)
...
GINKGO_MAX_NODES=${GINKGO_MAX_NODES:-3}
...
# Namespace configuration - allows running multiple test suites on same cluster
# Set different values for each repo to avoid conflicts:
#   cloudnative-pg:        OPERATOR_NAMESPACE=cnpg-system (default)
#   cloudnative-pg-gemini: OPERATOR_NAMESPACE=cnpg-system-gemini
OPERATOR_NAMESPACE=${OPERATOR_NAMESPACE:-cnpg-system}

# Test label filter - controls which tests run
# Examples:
#   GINKGO_LABEL_FILTER="dynamic-storage"  - PVC auto-resize tests (default)
#   GINKGO_LABEL_FILTER="snapshot"         - Volume snapshot tests
#   GINKGO_LABEL_FILTER="backup-restore"   - All backup/restore tests
#   GINKGO_LABEL_FILTER="smoke"            - Quick smoke tests
GINKGO_LABEL_FILTER=${GINKGO_LABEL_FILTER-dynamic-storage}
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --label|--label-filter)
      if [[ -n "${2:-}" ]]; then
        GINKGO_LABEL_FILTER="$2"; shift 2
      else
        fail "$1 requires a regex argument"
        exit 1
      fi
      ;;
    --label=*) GINKGO_LABEL_FILTER="${1#--label=}"; shift ;;
    --label-filter=*) GINKGO_LABEL_FILTER="${1#--label-filter=}"; shift ;;
    --depth)
      if [[ -n "${2:-}" ]]; then
        TEST_DEPTH="$2"; shift 2
      else
        fail "--depth requires a numeric argument"
        exit 1
      fi
      ;;
    --depth=*) TEST_DEPTH="${1#--depth=}"; shift ;;
    --focus)
      if [[ -n "${2:-}" ]]; then
...
print_victorialogs_hint() {
  ...
  info ""
  info "  {namespace=~\"dynamic-storage-.*|${OPERATOR_NAMESPACE}\"}"
  info ""
  info "To further filter for volume/resize issues:"
  info "  {namespace=~\"dynamic-storage-.*|${OPERATOR_NAMESPACE}\"} AND (_msg:~\"resize|volume|attach|disk|pvc\")"
  info ""
...
# 1b. Safety gate: Only allow running on infra cluster
ALLOWED_CONTEXTS=${ALLOWED_CONTEXTS:-"azure-infra|infra-cluster"}
if ! echo "${KUBE_CONTEXT}" | grep -qE "^(${ALLOWED_CONTEXTS})$"; then
  fail "E2E tests are only allowed to run on infra cluster contexts."
  fail "Current context: ${KUBE_CONTEXT}"
  fail "Allowed contexts: ${ALLOWED_CONTEXTS}"
  warn "To override (DANGER): export ALLOWED_CONTEXTS='.*'"
  exit 1
fi
ok "Context '${KUBE_CONTEXT}' is in allowed list"
...
# 2b. Ginkgo parallelism (up to 5 workers by default)
...
# 6. Check for multiple operator pods and auto-cleanup stale ones
OPERATOR_POD_NAMESPACES=$(kubectl get pods --all-namespaces -l "app.kubernetes.io/name=cloudnative-pg" --no-headers 2>/dev/null | awk '{print $1}' | sort -u)
OPERATOR_POD_COUNT=$(echo "${OPERATOR_POD_NAMESPACES}" | grep -c . 2>/dev/null || echo "0")

if [[ "${OPERATOR_POD_COUNT}" -gt 1 ]]; then
  warn "Found CNPG operator pods in ${OPERATOR_POD_COUNT} namespaces (tests expect exactly 1)"
  kubectl get pods --all-namespaces -l "app.kubernetes.io/name=cloudnative-pg" --no-headers 2>/dev/null
  # Auto-cleanup: delete operator namespaces other than the primary one
  for ns in ${OPERATOR_POD_NAMESPACES}; do
    if [[ "${ns}" != "${OPERATOR_NAMESPACE}" ]]; then
      warn "Deleting stale operator namespace: ${ns}"
  ...
  REMAINING=$(kubectl get pods --all-namespaces -l "app.kubernetes.io/name=cloudnative-pg" --no-headers 2>/dev/null | grep -c Running || echo "0")
  if [[ "${REMAINING}" -le 1 ]]; then
    ok "Stale operators cleaned up"
  else
    warn "Some stale operators may still be terminating - tests might need a retry"
  fi
elif [[ "${OPERATOR_POD_COUNT}" -eq 0 ]]; then
  warn "No CNPG operator pods found - tests may fail if operator is not deployed"
else
  ok "Single CNPG operator pod running"
fi

# 7. Check for stale minio deployment and ensure it's healthy
MINIO_PODS=$(kubectl get pods -n "${MINIO_NAMESPACE}" --no-headers 2>/dev/null | wc -l | tr -d ' ')
if [[ "${MINIO_PODS}" -gt 0 ]]; then
  MINIO_RUNNING=$(kubectl get pods -n "${MINIO_NAMESPACE}" --no-headers 2>/dev/null | grep -c Running || echo "0")
  if [[ "${MINIO_RUNNING}" -gt 0 ]]; then
    ok "MinIO: ${MINIO_RUNNING} pod(s) running in ${MINIO_NAMESPACE}"
  else
    warn "MinIO pods exist but none are Running in ${MINIO_NAMESPACE}"
    info "MinIO pod status:"
    kubectl get pods -n "${MINIO_NAMESPACE}" --no-headers 2>/dev/null
  fi
else
  info "No MinIO pods found in ${MINIO_NAMESPACE} (will be created by tests if needed)"
fi
...
info "Label filter: ${GINKGO_LABEL_FILTER:-<all>}"
if [ -n "${GINKGO_FOCUS}" ]; then
  info "Focus filter: ${GINKGO_FOCUS}"
fi
if [ -n "${TEST_DEPTH}" ]; then
  info "Test depth: ${TEST_DEPTH}"
fi
...
# Clean up test namespaces matching various patterns:
#   - dynamic-storage-*  (PVC auto-resize tests)
#   - stuck-replica-*    (snapshot recovery tests)
#   - volume-snapshot-*  (volume snapshot tests)
#   - declarative-snapshot-* (declarative backup tests)
#   - tablespaces-*      (tablespace tests)
#   - minio              (object store for backup tests)
ORPHAN_NS=$(kubectl get namespaces -o name 2>/dev/null | \
  grep -E "dynamic-storage-|stuck-replica-|volume-snapshot-|declarative-snapshot-|tablespaces-|^namespace/${MINIO_NAMESPACE}\$" | \
  cut -d/ -f2 || true)
if [ -n "${ORPHAN_NS}" ]; then
  ...
  # This prevents "object is being deleted" errors when tests try to
  # recreate these namespaces (especially minio for backup tests).
  info "Waiting for namespace termination to complete..."
  for ns in ${ORPHAN_NS}; do
    if kubectl get namespace "${ns}" >/dev/null 2>&1; then
      info "Waiting for namespace ${ns} to terminate..."
      kubectl wait --for=delete namespace/"${ns}" --timeout=120s 2>/dev/null || {
        warn "Namespace ${ns} still terminating after 120s - continuing anyway"
      }
    fi
  done
...
# Export required env vars for ginkgo subprocess
...
# Export test configuration for ginkgo subprocess
# TEST_DEPTH controls which tests run (0=Highest, 1=High, 2=Medium, 3=Low, 4=Lowest)
export TEST_DEPTH
# Volume snapshot class for snapshot tests
export E2E_DEFAULT_VOLUMESNAPSHOT_CLASS=${E2E_DEFAULT_VOLUMESNAPSHOT_CLASS:-csi-azuredisk-vsc}
# CSI storage class for PVC tests (re-export to ensure ginkgo sees it)
export E2E_DEFAULT_STORAGE_CLASS="${E2E_DEFAULT_STORAGE_CLASS}"
export E2E_CSI_STORAGE_CLASS=${E2E_CSI_STORAGE_CLASS:-${E2E_DEFAULT_STORAGE_CLASS}}
# Operator namespace for tests
export OPERATOR_NAMESPACE="${OPERATOR_NAMESPACE}"
# Skip operator pod stability checks (can cause false failures on long AKS runs)
export TEST_SKIP_OPERATOR_POD_STABILITY_CHECK=${TEST_SKIP_OPERATOR_POD_STABILITY_CHECK:-true}

# Log exported environment for debugging
info "Exported test environment:"
info "  TEST_DEPTH=${TEST_DEPTH}"
info "  E2E_DEFAULT_STORAGE_CLASS=${E2E_DEFAULT_STORAGE_CLASS}"
info "  E2E_CSI_STORAGE_CLASS=${E2E_CSI_STORAGE_CLASS}"
info "  E2E_DEFAULT_VOLUMESNAPSHOT_CLASS=${E2E_DEFAULT_VOLUMESNAPSHOT_CLASS}"
info "  OPERATOR_NAMESPACE=${OPERATOR_NAMESPACE}"
...
# Parallelism defaults to min(cluster nodes, 5). This increases throughput
# while avoiding over-subscribing small AKS clusters with too many concurrent
# disk attach/detach operations.
#
# To run with the maximum default parallelism:
#   GINKGO_NODES=5 hack/e2e/run-aks-e2e.sh --skip-build --skip-deploy
#
# On very small clusters or when troubleshooting flakiness, reduce workers:
#   GINKGO_NODES=1 hack/e2e/run-aks-e2e.sh --skip-build --skip-deploy
# Generate report filename from label filter (replace non-alphanumeric with underscore)
REPORT_NAME=$(echo "${GINKGO_LABEL_FILTER}" | tr -cs '[:alnum:]' '_' | sed 's/_$//')_report.json
...
RC=0
if [ -f "${ROOT_DIR}/tests/e2e/out/${REPORT_NAME}" ]; then
  jq -e -c -f "${ROOT_DIR}/hack/e2e/test-report.jq" \
    "${ROOT_DIR}/tests/e2e/out/${REPORT_NAME}" || RC=$?
fi
...
if [[ $RC_GINKGO != 0 ]]; then
  ...
  ok "All E2E tests passed! (label-filter: ${GINKGO_LABEL_FILTER})"
fi
