...
info "=== Running Dynamic Storage E2E Tests ==="
...
RC=0
if [ -f "${ROOT_DIR}/tests/e2e/out/dynamic_storage_report.json" ]; then
  jq -e -c -f "${ROOT_DIR}/hack/e2e/test-report.jq" \
    "${ROOT_DIR}/tests/e2e/out/dynamic_storage_report.json" || RC=$?
fi
