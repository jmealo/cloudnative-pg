...
if [ -z "$CLUSTER_NAME" ] || [ -z "$NAMESPACE" ]; then
  echo -e "${RED}Error: Cluster name and namespace are required.${NC}"
  usage
fi
...
# Display results
echo -e "\n${BLUE}=== Replication Slots Summary ===${NC}"
echo -e "${YELLOW}$(printf "%-30s %-10s %-8s %-15s %-15s %-12s %-10s %-20s %-10s" \
  "Slot Name" "Type" "Active" "Database" "Plugin" "WAL Retained" "WAL Status" "Instance" "Role")${NC}"
echo "======================================================================================================"
...
while IFS= read -r line; do
  ...
  # Format the output
  if [ "$active" = "t" ]; then
    active_display="${GREEN}ACTIVE${NC}"
  else
    active_display="${RED}INACTIVE${NC}"
  ...
  # Color code WAL status
  case "$wal_status" in
    "reserved")
      wal_status_display="${GREEN}$wal_status${NC}"
      ;;
    "extended"|"unreserved")
      wal_status_display="${YELLOW}$wal_status${NC}"
      ;;
    "lost")
      wal_status_display="${RED}$wal_status${NC}"
      ;;
    *)
      wal_status_display="$wal_status"
      ;;
  esac
  
  # Use echo -e to properly render color codes
  echo -e "$(printf "%-30s %-10s" "$slot_name" "$slot_type") $active_display $(printf "%-15s %-15s %-12s" "$database" "$plugin" "$formatted_size") $wal_status_display $(printf "%-20s %-10s" "$pod" "$role")"
...
if [ ${#INACTIVE_SLOTS[@]} -gt 0 ]; then
  echo -e "\n${YELLOW}Found ${#INACTIVE_SLOTS[@]} inactive replication slot(s) that can be cleaned up.${NC}"
  
  if [ "$DRY_RUN" = true ]; then
    echo -e "${BLUE}Dry-run mode: The following slots would be dropped:${NC}"
  ...
  if [ "$FORCE_DROP" = false ]; then
    echo -e "\n${YELLOW}Do you want to drop these inactive replication slots?${NC}"
  ...
  # Drop the slots
  echo -e "\n${BLUE}Dropping inactive replication slots...${NC}"
  DROPPED_COUNT=0
  FAILED_COUNT=0
